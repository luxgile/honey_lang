...
ParserResult<uptr<MetaDefExprAst>> Parser::parse_meta_expr(int &offset) {
  // Skip all new lines
...
void ParserError::print_error(const ParserError &error, std::string source) {
  auto pos = error.pos;
  ...
  std::println("{}{}error{}: {}", ansi_bold(), ansi_red(), ansi_reset(),
               error.msg);
  std::println("{}--- [{} {}-{}]{}", ansi_faint(), pos.line, pos.start, pos.end,
               ansi_reset());
  std::println("{}", line);
...
bool Parser::has_parsing_errors() { return errors.size() > 0; }

bool Parser::check_any_tokens(std::initializer_list<TokenKind> tokens,
                              int offset) {
...
bool Parser::check_tokens(std::initializer_list<TokenKind> tokens, int offset) {
...
void Parser::skip_until(std::initializer_list<TokenKind> kinds, int &offset) {
  while (!check_any_tokens(kinds, offset) && offset < (int)tk_queue.size())
    offset += 1;
}

void Parser::skip_until(TokenKind kind, int &offset) {
  while (!check_tokens({kind}, offset) && offset < (int)tk_queue.size())
    offset += 1;
}

void Parser::skip_all(std::initializer_list<TokenKind> kinds, int &offset) {
  while (check_any_tokens(kinds, offset) && offset < (int)tk_queue.size())
    offset += 1;
}

uptr<FileStmtAst> Parser::parse_file(std::string source, bool print_tokens) {
...
ParserResult<AstStatement> Parser::parse_file_statement(int &offset) {
  ...
  skip_until({NewLine, EoF}, offset);
  return ParserError::undefined_statement(get_tk(offset));
}

ParserResult<uptr<StructDefAst>> Parser::parse_struct_def(int &offset) {
  // Skip all new lines
  while (check_tokens({NewLine}, offset))
    offset += 1;
...
Parser::parse_fn_def(std::optional<AstTypeId> parent_struct, int &offset) {
  skip_all({NewLine}, offset);
...
ParserResult<AstTypeId> Parser::parse_type(int &offset) {
  bool is_ref = false;
...
ParserResult<uptr<EnumDefAst>> Parser::parse_enum_def(int &offset) {
  // Skip all new lines
  while (check_tokens({NewLine}, offset))
    offset += 1;
  ...
  while (!check_tokens({RBrace}, offset)) {
    ...
    if (check_tokens({Id, Comma}, offset)) {
      ...
      field_types.push_back(AstNamedType{stmt_type->get_name(), stmt_type_id});
...
Parser::consume_expressions(int &offset,
...
Parser::parse_var_assign(int &offset,
                         std::initializer_list<TokenKind> halt_tokens) {
...
ParserResult<uptr<VarDefStmtAst>> Parser::parse_var_decl(int &offset) {
  if (!check_tokens({Id, Colon, Id}, offset) || get_tk(offset + 2).value != "=")
    return {};
...
ParserResult<uptr<ReturnStmtAst>> Parser::parse_return(int &offset) {
...
ParserResult<AstStatement> Parser::parse_statement(int &offset) {
  ...
  auto last_expr = consume_expressions(offset, {NewLine, RBrace});
  if (!last_expr)
...
ParserResult<AstExpression> Parser::parse_expr(int &offset) {
  ...
  if (auto mem_acc = parser_member_access_expr(offset, inner_expr.get_res())) {
...
ParserResult<AstExpression> Parser::parse_inner_expr(int &offset) {
...
ParserResult<uptr<EnumExprAst>> Parser::parse_enum_expr(int &offset) {
...
Parser::parser_member_access_expr(int &offset, AstExpression &base_expr) {
...
ParserResult<uptr<SingleMatchExprAst>>
Parser::parse_single_match_expr(int &offset) {
...
ParserResult<uptr<IfExprAst>> Parser::parse_if_expr(int &offset) {
...
ParserResult<uptr<ForExprAst>> Parser::parse_loop_expr(int &offset) {
...
Parser::parse_call_expr(int &offset, std::optional<AstTypeId> parent) {
  LOG("parsing call");
  auto identifier = get_tk(offset).value;
  ...
  for (auto fn : fns) {
    ...
    if (fn_ty->is_varadic()) {
      ...
      while (true) {
        ...
        if (!fn_ty->is_varadic() && (i >= (int)fn_ty->get_su_args().size() ||
    ...
    return std::make_unique<CallExprAst>(
        fn_ty->get_name(), fn, std::move(prefix_args), std::move(suffix_args));
  }
...
ParserResult<std::unique_ptr<StructExprAst>>
Parser::parse_struct_expr(int &offset) {
...
Parser::parse_index_expr(int &offset, AstExpression &base) {
...
ParserResult<std::unique_ptr<ArrayExprAst>>
Parser::parse_array_expr(int &offset) {
  if (!check_tokens({LBracks}, offset))
    return {};
  offset += 1;
...
Parser::parse_struct_assigments(int &offset) {
...
ParserResult<std::unique_ptr<BodyExprAst>> Parser::parse_fn_body(int &offset) {
  ...
  while (!check_any_tokens({RBrace, EoF}, offset)) {
...
Parser::parse_fn_header(std::string header_id,
                        std::optional<AstTypeId> parent_struct, int &offset) {
...
Parser::parse_fn_args(std::optional<AstTypeId> parent_struct, int &offset) {
...
ParserResult<uptr<ArgDefAst>> Parser::parse_arg_def(int &offset) {
  // Varadic argument
